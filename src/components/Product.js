...
const Product = ({id, key, title, description, price, quantity, rating, image, isVeg, category, changeProductCount, toggleModal, addProduct, cart}) => {
  const [productQuantity, setProductQuantity] = useState(1);
  const isAlreadyAddedProduct = cart.some((item)=> item.id === id);
        ...
        <div className='product-info'>
          <div className='title-section'>
            ...
            {isAlreadyAddedProduct ? (
              <div disabled={isOutOfStock} className={`${isOutOfStock ? 'btn-disabled quantity qty-btn' : 'quantity qty-btn'}`}>
                <span 
                className='minus-sign'
                onClick={
                  !isOutOfStock ? () => changeProductCount(id, 'decrement')
                  : null
                }
                >
                  &#8722;
                ...
                className='plus-sign'
                onClick={
                  !isOutOfStock ? () => changeProductCount(id, 'increment')
                  : null
                }
                >
                  &#43;
              ...
              type='button'
              className={`${isOutOfStock ? 'btn-disabled add-btn' : 'add-btn'}`}
